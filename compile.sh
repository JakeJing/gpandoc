...
# pandoc --template=templates/simple.latex --pdf-engine=xelatex -F pandoc-citeproc -M secPrefix=Section -N main.txt -o mainr.tex 2> pandoc-errors.txt

pandoc --lua-filter=lua-filters/scholarly-metadata/scholarly-metadata.lua --lua-filter=lua-filters/author-info-blocks/author-info-blocks.lua --template=./templates/simple.latex --pdf-engine=xelatex -F /usr/local/bin/glosses.py -M secPrefix=Section -N main.txt -o mainr.tex 2> pandoc-errors.txt
